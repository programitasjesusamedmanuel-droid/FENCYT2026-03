...
},
{
  ...
  /* =======================
     CRONOGRAMA: generarDiasCronograma, initCronograma, actualizarProgreso
     ======================= */

  function generarDiasCronograma(recetaId, dias) {
    const n = Number(dias) || 0;
    let html = '<div class="dias-grid" role="group" aria-label="Días del cronograma">';
    for (let i = 1; i <= n; i++) {
      html += `<button
          type="button"
          class="dia"
  ...
  function initCronograma(container) {
    const cont = (typeof container === 'string') ? document.querySelector(container) : container;
    if (!cont) return;

    const dias = Array.from(cont.querySelectorAll('.dia'));
    const recetaIdAttr = cont.getAttribute('data-receta-id') || (dias[0] && dias[0].getAttribute('data-receta-id'));
    const recetaId = recetaIdAttr ? parseInt(recetaIdAttr, 10) : null;
    const total = dias.length || 0;
    const root = cont.closest('#detalle-receta') || cont.closest('.detalle-section') || cont.closest('.modal') || document;
    const fill = root.querySelector('.progreso-fill');
    const text = root.querySelector('.progreso-text');
    const usedEl = root.querySelector('.dias-usados');
    ...
    dias.forEach(btn => {
      ...
      const isUsed = cronograma.includes(diaNum);
      btn.setAttribute('aria-pressed', isUsed ? 'true' : 'false');
      btn.classList.toggle('dia--done', isUsed);

      if (!btn._cronogramaBound) {
        btn._cronogramaBound = true;
        btn.addEventListener('click', () => {
          const pressed = btn.getAttribute('aria-pressed') === 'true';
          btn.setAttribute('aria-pressed', (!pressed).toString());
          btn.classList.toggle('dia--done', !pressed);

          let stored = JSON.parse(localStorage.getItem(`cronograma-${recetaId}`) || '[]');
          if (!Array.isArray(stored)) stored = [];
          const d = parseInt(btn.getAttribute('data-dia'), 10);
          if (pressed) stored = stored.filter(x => x !== d);
          else if (!stored.includes(d)) stored.push(d);
          localStorage.setItem(`cronograma-${recetaId}`, JSON.stringify(stored));
          actualizarProgreso(recetaId, total, root);
        });
  ...
  function actualizarProgreso(recetaId, totalDays, rootScope) {
    const root = rootScope || document;
    const cronograma = JSON.parse(localStorage.getItem(`cronograma-${recetaId}`) || '[]');
    const used = Array.isArray(cronograma) ? cronograma.length : 0;
    const percent = totalDays ? Math.round((used / totalDays) * 100) : 0;
    ...
    if (text) {
      if (typeof window.t === 'function') text.innerHTML = window.t('progress', { percent });
      else text.textContent = `Progreso: ${percent}%`;
    }
...
${t('recipe.schedule.info', {days: receta.tiempoRecomendado})}
  ...
  function renderComentarios(comments) {
    if (!comments || !comments.length) return `<p>${t('comments.no')}</p>`;
    return comments.map(c => `<div class="comentario"><strong>${safeText(c.autor)}</strong> <em>${safeText(c.fecha)}</em><p>${safeText(c.texto)}</p></div>`).join('');
  }
  ...
  function mostrarDetalleReceta(recetaId) {
    ...
    detalleReceta.innerHTML = `
      ...
      <div class="cronograma">
        ...
        <div class="dias-container" data-receta-id="${receta.id}">
        ...
        <div class="cronograma-info">
          <p>${t('recipe.schedule.days', {used: '<span class="dias-usados">0</span>', total: receta.tiempoRecomendado})}</p>
          <p class="progreso-text">${t('progress', {percent: 0})}</p>
        </div>
      </div>
    ...
    const diasContainer = detalleReceta.querySelector('.dias-container');
    if (diasContainer) initCronograma(diasContainer);

    detalleReceta.querySelectorAll('.dia').forEach(dia => {
      if (!dia._modalBound) {
        dia._modalBound = true;
        dia.addEventListener('click', () => {
          const recetaIdAttr = dia.getAttribute('data-receta-id');
          const recetaIdN = recetaIdAttr ? parseInt(recetaIdAttr, 10) : receta.id;
          const diaNumero = parseInt(dia.getAttribute('data-dia'), 10);
          toggleDiaCronograma(recetaIdN, diaNumero);
          actualizarProgreso(recetaIdN, receta.tiempoRecomendado, detalleReceta);
        });
      }
    });
    ...
    actualizarProgreso(receta.id, receta.tiempoRecomendado, detalleReceta);
  }
...
(function() {
  ...
  // ---------- APP DATA ----------
  const recetas = window.__rc_app__?.recetas || [];
